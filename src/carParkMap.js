...
function getRandomTimer() {
    min = Math.ceil(1800);
    max = Math.floor(28800);
    return Math.floor(Math.random() * (max - min + 1) + min); // The maximum is inclusive and the minimum is inclusive
  }

  function startTimer(randomTime, slot) {
    randomTime--;
    
    var hours = Math.floor(randomTime / (60*60));
    var minutes = Math.floor((randomTime % (60*60)) / 60);
    var seconds = Math.floor(randomTime %  60);
  
    if (randomTime < 0) {
        document.getElementById('timer'+(slot+1).toString()).innerHTML = "0h 0m 0s";
      clearInterval(timerInterval);
      return;
    }

    document.getElementById('timer'+(slot+1).toString()).innerHTML = hours + "h "+ minutes + "m " + seconds + "s ";
  }
...
function carexit(slot){
    if(!parklock){
        ...
        parklock = true;
        document.getElementById('timer'+(slot+1).toString()).innerHTML ="";
        document.getElementById('slot-'+(slot+1).toString()).style.transitionDelay = '1s';
...
function carenter(slot){
    ...
    if(!document.getElementById('car'+(slot).toString()) && !parklock){
        ...
        var randomT = getRandomTimer();  //test
        //setInterval(startTimer(randomT, slot), 1000); //test
        var hours = Math.floor(randomT / (60*60));
        var minutes = Math.floor((randomT % (60*60)) / 60);
        var seconds = Math.floor(randomT %  60);
        document.getElementById('timer'+(slot+1).toString()).innerHTML = hours + "h "+ minutes + "m " + seconds + "s ";

        document.getElementById('slot-'+(slot+1).toString()).style.transitionDelay = '2s';
